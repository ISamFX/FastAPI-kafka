# src/Consumer_api/app/tests/test_consumer_versions.py
import sys
import os
import requests
import time
...
class VersionTester:
    MESSAGE_TYPES = {
        1: ["number_id_v1_json", "number_id_v1_proto"],
        2: ["string_id_v1_json", "string_id_v1_proto", "avro_message"]
    }
        ...
        try:
            if version == 1:
                if not isinstance(data, list):
                    return False
                return all('id' in msg for msg in data)
            else:
                if 'messages' not in data:
                    return False
                return all('id' in msg for msg in data['messages'])
        except:
            return False

    def test_version(self, version, message_type=None):
        ...
        try:
            ...
            if response.status_code == 200:
                is_valid = self.validate_response(version, response.json())
                if is_valid:
                    ...
                    logger.info(f"[v{version}/{message_type or 'all'}] Valid response in {duration:.2f}s")
                    return True
                else:
            ...
            logger.error(f"[v{version}/{message_type or 'all'}] Error: {str(e)}")
            return False
    ...
    def run_test(self, num_requests, versions, workers, duration=None):
        ...
        for v in versions:
            ...
            for msg_type in self.MESSAGE_TYPES.get(v, []):
                test_cases.append((v, msg_type))
        
        def worker():
            requests_made = 0
            while requests_made < num_requests if not duration else True:
                for version, msg_type in test_cases:
                    if duration and (datetime.now() - self.stats['start_time']).total_seconds() > duration:
                        return
                        
                    self.test_version(version, msg_type)
                    requests_made += 1
                    if not duration and requests_made >= num_requests:
                        return
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Kafka Consumer Version Tester')
